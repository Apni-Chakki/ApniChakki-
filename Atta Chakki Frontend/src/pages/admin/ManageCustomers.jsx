  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">{t('Manage Customers')}</h1>
          <p className="text-sm text-gray-500">{t('View customer details, configure VIP privileges, and manage account statuses.')}</p>
        </div>
        <Button
          onClick={() => {
          ...
          className="bg-purple-600 hover:bg-purple-700 text-white font-medium flex items-center gap-1.5 shadow-sm sm:self-center"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center space-x-4 border-l-4 border-blue-500 shadow-sm">
          <div className="p-3 bg-blue-100 rounded-full text-blue-600">
            <User className="h-6 w-6" />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">{t('Total Customers')}</p>
            <p className="text-2xl font-bold text-gray-900">{totalCustomersCount}</p>
          </div>
        </Card>

        <Card className="p-4 flex items-center space-x-4 border-l-4 border-green-500 shadow-sm">
          <div className="p-3 bg-green-100 rounded-full text-green-600">
            <UserCheck className="h-6 w-6" />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">{t('Active Accounts')}</p>
            <p className="text-2xl font-bold text-gray-900">{activeCustomersCount}</p>
          </div>
        </Card>

        <Card className="p-4 flex items-center space-x-4 border-l-4 border-purple-500 shadow-sm">
          <div className="p-3 bg-purple-100 rounded-full text-purple-600">
            <Award className="h-6 w-6" />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">{t('VIP Customers')}</p>
            <p className="text-2xl font-bold text-gray-900">{vipCustomersCount}</p>
          </div>
        </Card>

        <Card className="p-4 flex items-center space-x-4 border-l-4 border-amber-500 shadow-sm">
          <div className="p-3 bg-amber-100 rounded-full text-amber-600">
            <Sparkles className="h-6 w-6" />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">{t('Total Spent')}</p>
            <p className="text-2xl font-bold text-gray-900">Rs. {totalSalesAmount.toLocaleString('en-PK', { minimumFractionDigits: 2 })}</p>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6 shadow-sm">
        ...
        {filteredCustomers.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">{t('No customers found matching your search.')}</p>
          </div>
        ) : (
      ...
      <Dialog open={isManagePrivilegesOpen} onOpenChange={setIsManagePrivilegesOpen}>
        <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-2 mt-4">
            ...
            <div className="divide-y divide-gray-100 max-h-48 overflow-y-auto pr-1">
              {privileges.length === 0 ? (
                ...
                privileges.map((priv) => (
                  <div key={priv.id} className="py-2.5 flex items-center justify-between group">
                    <div className="space-y-0.5">
          ...
          <DialogFooter className="pt-4 border-t border-gray-100">
            <Button
              className="bg-gray-950 hover:bg-gray-900 text-white font-medium"
