...
export function LiveTrackingMap() {
  ...
  // ─── Draw actual GPS trail on map ───
  useEffect(() => {
    if (!mapRef.current || !window.google) return;

    if (orderDetail && orderDetail.trail && orderDetail.trail.length > 0) {
      if (trailPolylineRef.current) {
        trailPolylineRef.current.setMap(null);
      }

      const path = orderDetail.trail.map(point => ({
        lat: parseFloat(point.latitude),
        lng: parseFloat(point.longitude)
      }));

      // Draw the actual path driver has taken
      trailPolylineRef.current = new window.google.maps.Polyline({
        path,
        geodesic: true,
        strokeColor: '#f59e0b', // Amber/orange color
        strokeOpacity: 0.8,
        strokeWeight: 5,
        map: mapRef.current,
        zIndex: 50,
          ...
          {selectedOrder && orderDetail && orderDetail.location && (
            <Card className="p-4 bg-purple-50/30 border-purple-200">
              ...
              <div className="text-xs space-y-1.5 text-foreground">
                ...
                {orderDetail.trail && (
                  <div className="mt-2 space-y-1 p-2 bg-white/50 rounded-md border border-purple-100">
                    <p className="text-muted-foreground mb-2">
                      🗺️ {orderDetail.trail.length} {t('GPS points recorded')}
                    </p>
                    <div className="flex items-center gap-2 text-xs font-medium">
                      <div className="w-4 h-1.5 bg-blue-500 rounded-full"></div> {t('Planned Route')}
                    </div>
                    <div className="flex items-center gap-2 text-xs font-medium">
                      <div className="w-4 h-1.5 bg-amber-500 rounded-full"></div> {t('Actual Path Taken')}
                    </div>
                  </div>
                )}
              </div>
