<?php
// get_yesterday_pending.php - puray dino k orders ki details nikal rahe han hum yahan
// admin ko dikhanay k liye takey wo select kar sakay
...
try {
    ...
    // pending orders dhoond rahe han puray dino k
    ...
    while ($row = $result->fetch_assoc()) {
        // customer ka naam aur phone nikal rahe han
        $user_id = $row['user_id'];
        if ($user_id > 0) {
        ...
        // order k items nikal rahe han details k liye
        $order_id = $row['id'];
        $items = [];
