...
try {
    ...
    if ($result) {
        while($row = $result->fetch_assoc()) {
            ...
            $order_id = $row['id'];
            $items = [];
            $item_res = $conn->query("SELECT quantity, product_id, price_at_purchase, original_price, is_cleaning, is_grinding, is_weight_pending FROM order_items WHERE order_id = '$order_id'");
