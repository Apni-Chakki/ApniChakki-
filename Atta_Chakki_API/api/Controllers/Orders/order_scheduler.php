...
// hamesha pakistan ka time use karna hai hum ne
...
function getScheduleAvailability($conn, $estimated_weight_kg = 1) {
    // step 1: hours nikal rahe han setting se
    ...
    // step 2: TIME CHECK — kahin shop band hone wali to nahi?
    ...
    // step 3: LOAD CHECK — kya aaj k orders full ho gaye hain?
    $today_count = getActiveOrderCount($conn, $today);
    $load_blocked = ($today_count >= $max_orders);
    
    // step 4: CAPACITY CHECK — kya ye order aaj khatam ho sakey ga?
    ...
    if ($last_completion) {
        ...
        // agar pichla order purana hai to abhi se shuru karo, warna pichle order k khatam hone par
        $start_time = ($now > $last_dt) ? clone $now : $last_dt;
    } else {
        $opening_dt = new DateTime($today . ' ' . $opening_time . ':00');
        // agar dukan abhi khuli nahi to opening time pe shuru karo, warna abhi se
    ...
    // yahan fesla kar rahe han kya karna hai
    ...
    if ($push_to_tomorrow) {
        // kal ki ETA calculate kar rahe han
...
function scheduleOrder($conn, $order_id) {
    ...
    if (!$force_tomorrow) {
        ...
        // step 7: khatam hone ka time calculate kar rahe han
        $eta = clone $start_time;
        $eta->modify("+{$processing_minutes} minutes");
        
        // step 8: check kar rahe han shop band hone se pehle ho jaye ga?
        if ($eta <= $closing_dt) {
            // aaj hi ho jaye ga
            $assigned_date = $today;
            $estimated_completion = $eta->format('Y-m-d H:i:s');
            ...
            // aaj nahi ho sakta — rush zyada hai
            $force_tomorrow = true;
        }
    }
    ...
    if ($force_tomorrow) {
        ...
        // kal k opening time se hisab laga rahe han
        $tomorrow_last = getLastCompletionTime($conn, $tomorrow);
        ...
        // kyun kal pe gaya wo reason save kar rahe han
        if ($time_blocked) {
    ...
    // step 10: db me order update kar rahe han
...
function getCapacityInfo($conn, $date) {
    ...
    // total kitne minutes hain din me (khulne se band hone tak)
    ...
    // kitne minutes pehle se book hain wo nikal rahe han
