...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($existing) {
            ...
            $stmt = $conn->prepare(
                "INSERT INTO tracking_tokens (order_id, token, expires_at) VALUES (?, ?, ?)"
            );
            $stmt->bind_param("iss", $order_id, $token, $expires_at);
            $stmt->execute();
        }
        ...
        $whatsapp_message = "🌟 *Apni Chakki — Order On The Way!* 🌟\n\n"
            . "Assalam-o-Alaikum! Your order *#{$order_id}* has been dispatched and is currently on its way to you. 🚚💨\n\n"
            . "📦 *Order Amount:* Rs. " . number_format($order['total_amount'] ?? 0) . "\n"
            . "🧑‍💼 *Rider:* " . ($driver_name ?: 'Apni Chakki Driver') . "\n\n"
            . "📍 *Track Your Order Live:*\n"
            . $tracking_url . "\n\n"
            . "Click the link above to view your rider's live location on the map. Please keep your phone nearby. \n\n"
        ...
        if (isset($_GET['token'])) {
            ...
            echo json_encode([
                ...
                "driver_name" => $result['assigned_driver'],
