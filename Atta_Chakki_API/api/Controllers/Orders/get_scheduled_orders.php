...
try {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        while($i = $item_res->fetch_assoc()) {
             ...
             // Fetch dynamic customizations for this order item
             $oi_id = $i['id'];
             $cust_res = $conn->query("SELECT option_name, option_price FROM order_item_customizations WHERE order_item_id = '$oi_id'");
             $customizations = [];
             if ($cust_res) {
                 while ($c = $cust_res->fetch_assoc()) {
                     $customizations[] = ['option_name' => $c['option_name'], 'option_price' => floatval($c['option_price'])];
                 }
             }
