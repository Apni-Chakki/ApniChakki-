    ...
    if ($stmt->execute()) {
        ...
        if (isset($data['customizations']) && is_array($data['customizations'])) {
            ...
            foreach ($data['customizations'] as $cust) {
                ...
                $cust_stmt->bind_param("iisdi", $id, $linked_product_id, $opt_name, $opt_price, $sort_order);
                $cust_stmt->execute();
            }
            $cust_stmt->close();
        }
        ...
        if (isset($data['mix_items']) && is_array($data['mix_items'])) {
            ...
            foreach ($data['mix_items'] as $mix) {
                ...
                $mix_stmt->bind_param("iisddi", $id, $product_ingredient_id, $item_name, $price_per_kg, $default_ratio, $sort_order);
                $mix_stmt->execute();
            }
            $mix_stmt->close();
        }
