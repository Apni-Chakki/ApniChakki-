...
class WalletService {
    ...
    public static function updateBankDetails($conn, array $data) {
        ...
        if (empty($bank_name) || empty($account_name) || empty($account_number)) {
            return ["success" => false, "message" => "Bank Name, Account Title, and Account Number are required."];
        }
    ...
    public static function getBusinessBalance($conn) {
        ...
        if ($result->num_rows === 0) {
            $stmt->close();
            return ["success" => true, "balance" => 0, "account_name" => "No account configured"];
        }
    ...
    public static function rejectBankPayment($conn, array $data) {
        ...
        try {
            ...
            if (!empty($payment['customer_email'])) {
                ...
                $emailHelper = __DIR__ . '/../../utils/email_helper.php';
